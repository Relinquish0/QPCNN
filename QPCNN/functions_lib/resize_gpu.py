...
# 输入数据x的尺寸变化过程形如: (39601, 4, 64) -> (64, 4, 39601) -> (1, 64, 4, 39601)
def rect34(x):
...
def dia_symmetry(x):
    mould199_199 = torch.zeros((x.shape[0], x.shape[1], x.shape[2], 199, 199)).cuda()
    # 横轴为c3轴，纵轴为c5轴
    index = 0
...
def dia_asymmetry(x):
    mould19900 = torch.zeros((x.shape[0], x.shape[1], x.shape[2], 19900)).cuda()
    index = 0
    for col in range(0, 199):  # 0-198列
        for row in range(col + 1):  # 0-(col-1)行，共col行
